...
## 🗂️ 模組架構 (Architecture)
```text
├── index.html       # 應用程式入口、HUD、抽屜式選單與模態框結構
├── style.css        # 工業科幻 UI、iOS 安全區適配、響應式佈局 (Mobile RWD)
├── main.js          # Three.js 3D 渲染管線、自訂 Shader 與後處理
├── physics.js       # 1.5D Crank-Nicolson 輸運、真實裝置預設庫、超導失超模型
├── controller.js    # 遊戲狀態機、生涯成就、歷史標竿對標、音效觸覺調度
├── ui.js            # 雙語多語言系統 (i18n)、抽屜切換、虛擬湯姆遜散射繪製
 ...
 * 詳閱 法律免責聲明 (DISCLAIMER.md)
 * 詳閱 AI 與出口管制合規評估 (COMPLIANCE.md)
 * 詳閱 數據隱私保護政策 (PRIVACY.md)
